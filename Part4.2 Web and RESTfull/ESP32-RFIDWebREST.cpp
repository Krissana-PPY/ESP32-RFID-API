...
const char* ssid = "ESP32-4401";
const char* password = "1q2w3e4r5t@coe";
...
void setup() {
  ...
  pinMode(RELAY_PIN, OUTPUT); // Initialize the relay pin as an output
    // curl -X POST http://ip_address_ESP32:88/api/add --data-urlencode "uid=73A0301D" --data-urlencode "owner=NAME"
  server.on("/api/add", HTTP_POST, []() {
    if (!server.hasArg("uid") || !server.hasArg("owner")) {
      server.send(400, "application/json", "{\"error\": \"Missing UID or Owner\"}");
      return;
    }
    String uid = server.arg("uid");
    String owner = server.arg("owner");

    File file = SPIFFS.open("/UIDs.csv", "a");
    if (file) {
      file.println(uid + "," + owner);
      file.close();
      server.send(200, "application/json", "{\"message\": \"UID added\"}");
  ...
  // curl -X POST http://ip_address_ESP32/api/delete --data-urlencode "uid=73A0301D"
  server.on("/api/delete", HTTP_POST, []() {
    ...
    File outFile = SPIFFS.open("/UIDs.csv", "w");
    if (outFile) {
      outFile.print(newContent);
      outFile.close();
      server.send(200, "application/json", "{\"message\": \"UID deleted\"}");
      Serial.println(uidToDelete + " deleted from UIDs.csv");
    } else {
      server.send(500, "application/json", "{\"error\": \"Failed to update file\"}");
    }
  });

  // curl -X GET http://ip_address_ESP32/api/request
  server.on("/api/request", HTTP_GET, []() {
    File file = SPIFFS.open("/UIDs.csv", "r");
    if (!file) {
      server.send(500, "application/json", "{\"error\": \"Failed to open file\"}");
      return;
  ...
  // curl -X POST http://ip_address_ESP32/api/open
  server.on("/api/open", HTTP_POST, []() {
    openDoor();
    server.send(200, "application/json", "{\"message\": \"Door opened\"}");
